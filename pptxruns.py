...
# ── Default SEGA Blue template embedded as base64 ────────────────────────────
# Source: SOA-HD_Template_Blue.pptx
# Loaded at startup so no file upload is needed for the standard SEGA blue look.
...
del _base64  # clean up namespace
...
def _write_speaker_notes(slide, text: str) -> None:
    ...
    for idx, s in enumerate(slides_data, start=1):
        ...
        if stype == "chart":
            renderer(prs, s, C)
            # Get the last added slide for notes
            _slide_for_notes = prs.slides[-1]
        else:
            renderer(slide, s, idx, total, C)
            _slide_for_notes = slide
        _write_speaker_notes(_slide_for_notes, s.get("speaker_notes") or s.get("notes") or "")
...
with st.sidebar:
    ...
    _uploaded_tpl = st.session_state.get("template_upload")
    if _uploaded_tpl:
        st.markdown(
            f"<div style='font-size:.75rem;color:#22d3ee;margin-bottom:.3rem'>"
            f"✓ Using uploaded: <b>{_uploaded_tpl.name}</b></div>",
            unsafe_allow_html=True,
        )
    else:
        st.markdown(
            "<div style='font-size:.75rem;color:#60a5fa;margin-bottom:.3rem'>"
            "✓ Using default: <b>SEGA Blue (SOA-HD)</b></div>",
            unsafe_allow_html=True,
        )

    template_file = st.file_uploader(
        "Override template (.pptx)",
        type=["pptx"],
        label_visibility="visible",
        help="Upload a branded .pptx to override the default SEGA blue template. "
             "The app will extract its colours, fonts, and layouts.",
        key="template_upload",
    )

    # Theme is only used when no template is active (fallback scratch-build)
    theme_preset = "SEGA Blue — Corporate Executive"  # kept for run_pipeline signature
